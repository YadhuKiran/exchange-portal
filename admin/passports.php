...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verify_csrf();
    verify_compliance_record('passports', (int)$_POST['id'], $_POST['status'], (int)current_user()['id']);
    flash('success', 'Passport updated.');
    redirect('/admin/passports.php');
}

$pageTitle = 'Passports';
$activeNav = 'passports';
$rows = enterprise_tables_ready() ? db()->query(
    'SELECT p.*, u.first_name, u.last_name FROM passports p
     JOIN students s ON s.id=p.student_id JOIN users u ON u.id=s.user_id ORDER BY p.updated_at DESC'
)->fetchAll() : [];
...
<div class="mb-6 flex items-center justify-between gap-4">
    <div class="relative">
        ...
        <input type="text" id="searchPassports" data-table-search="passportsTable" placeholder="Search passports..." class="w-full sm:w-64 pl-9 pr-3 py-2 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 placeholder-slate-400 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all">
    </div>
...
<div class="table-wrapper bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
    <div class="overflow-x-auto">
        <table id="passportsTable" data-table-paginate="15" class="sortable w-full text-sm">
            <thead class="bg-slate-50/80 text-slate-500 text-xs uppercase tracking-wider font-semibold"><tr>
                <th class="px-6 py-3.5 text-left w-12">#</th><th class="px-6 py-3.5 text-left" data-sort="student">Student</th><th class="px-6 py-3.5 text-left" data-sort="number">Passport No.</th>
                <th class="px-6 py-3.5 text-left" data-sort="expiry">Expiry</th><th class="px-6 py-3.5 text-left" data-sort="status">Status</th><th class="px-6 py-3.5 text-right">Actions</th>
            </tr></thead>
            ...
            <tr class="table-row-hover">
                ...
                <td class="px-6 py-4"><span class="font-mono text-xs"><?= e($r['passport_number']) ?></span></td>
    ...
    <?php if (!$rows): ?><div class="p-12 text-center text-slate-500">No passport records found.</div><?php endif; ?>
</div>
