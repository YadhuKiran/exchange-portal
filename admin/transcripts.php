...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verify_csrf();
    if (verify_compliance_record('transcripts', (int)$_POST['id'], $_POST['status'], (int)current_user()['id'])) {
        flash('success', 'Transcript updated.');
    }
    redirect('/admin/transcripts.php');
}

$pageTitle = 'Transcripts';
$activeNav = 'transcripts';
$rows = enterprise_tables_ready() ? db()->query(
    'SELECT t.*, u.first_name, u.last_name FROM transcripts t JOIN students s ON s.id=t.student_id JOIN users u ON u.id=s.user_id ORDER BY t.created_at DESC'
)->fetchAll() : [];
...
<div class="mb-6 flex items-center justify-between gap-4">
    <div class="relative">
        ...
        <input type="text" id="searchTranscripts" data-table-search="transcriptsTable" placeholder="Search transcripts..." class="w-full sm:w-64 pl-9 pr-3 py-2 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 placeholder-slate-400 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all">
    </div>
...
<div class="table-wrapper bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
    <div class="overflow-x-auto">
        <table id="transcriptsTable" data-table-paginate="15" class="sortable w-full text-sm">
            <thead class="bg-slate-50/80 text-slate-500 text-xs uppercase tracking-wider font-semibold"><tr>
                <th class="px-6 py-3.5 text-left w-12">#</th><th class="px-6 py-3.5 text-left" data-sort="student">Student</th><th class="px-6 py-3.5 text-left" data-sort="institution">Institution</th>
                <th class="px-6 py-3.5 text-left" data-sort="gpa">GPA</th><th class="px-6 py-3.5 text-left" data-sort="status">Status</th><th class="px-6 py-3.5 text-right">Actions</th>
            </tr></thead>
            ...
            <tr class="table-row-hover">
                ...
                <td class="px-6 py-4"><span class="font-medium text-slate-900"><?= e($r['institution_name']) ?></span><?php if ($r['degree_program']): ?><p class="text-xs text-slate-500"><?= e($r['degree_program']) ?></p><?php endif; ?></td>
                <td class="px-6 py-4"><?= $r['gpa'] ? '<span class="font-semibold">' . e($r['gpa']) . '</span>' : '<span class="text-slate-400">—</span>' ?></td>
    ...
    <?php if (!$rows): ?><div class="p-12 text-center text-slate-500">No transcripts found.</div><?php endif; ?>
</div>
