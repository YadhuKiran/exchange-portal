...
$rows = db()->prepare(
    "SELECT * FROM notifications WHERE user_id = ? AND (university_id IS NULL OR university_id = ?)
     ORDER BY created_at DESC"
);
$rows->execute([$userId, $uniId]);
$rows = $rows->fetchAll();
...
<div class="mb-6 flex items-center justify-between">
    <div><h1 class="text-lg font-bold text-slate-900">Notifications</h1><p class="text-xs text-slate-500">Alerts and updates</p></div>
</div>
