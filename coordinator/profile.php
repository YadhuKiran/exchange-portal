...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($pw) {
        if ($pw !== ($_POST['confirm_password'] ?? '')) {
            ...
            redirect('/coordinator/profile.php');
        }
        ...
        db()->prepare('UPDATE coordinators SET department=? WHERE user_id=?')
            ->execute([trim($_POST['department'] ?? ''), $uid]);
...
?>
        <div class="mb-6 pb-4 border-b border-slate-100">
            ...
            <p class="text-xs text-slate-500"><?= e($coord['university_name'] ?? '') ?></p>
        </div>
        <form method="post" enctype="multipart/form-data" class="space-y-5">
            ...
            <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Department</label>
                <input name="department" value="<?= e($coord['department'] ?? '') ?>" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all"></div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="<?= url('/coordinator/index.php') ?>" class="px-6 py-2.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-all">Cancel</a>
            </div>
        </form>
    </div>
</div>
