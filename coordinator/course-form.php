...
$course = null;
if ($id) {
    $stmt = db()->prepare('SELECT * FROM courses WHERE id = ? AND university_id = ?');
    $stmt->execute([$id, $uniId]);
    $course = $stmt->fetch();
    if (!$course) { flash('error', 'Course not found.'); redirect('/coordinator/courses.php'); }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    redirect('/coordinator/courses.php');
}
    ...
    <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-8 card-hover">
        ...
        <form method="post" class="space-y-5">
            ...
            <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Course title</label>
                <input name="title" required value="<?= e($course['title'] ?? '') ?>" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all" placeholder="e.g. Introduction to CS"></div>
            <div class="grid grid-cols-2 gap-4">
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="<?= url('/coordinator/courses.php') ?>" class="px-6 py-2.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-all">Cancel</a>
            </div>
        </form>
    </div>
</div>
